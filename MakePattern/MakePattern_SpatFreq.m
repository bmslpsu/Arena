function [] = MakePattern_SpatFreq(freq,root,playPat,savePat)
%---------------------------------------------------------------------------------------------------------------------------------
...
for jj = 1:pattern.y_num
    ...
    for kk = barwidth(jj):barwidth(jj):pattern.x_num
        Pats(:,pp:kk, 1, qq) = [zeros(pattern.y_panel,barwidth(jj)/2) ones(pattern.y_panel,barwidth(jj)/2)];
        pp = kk + 1;
    end
    qq = qq + 1;
end
...
if playPat
    ...
    tic           
    for jj = 1:2 % how many time to loop pattern
...
if savePat
	...
	str = [root '\Pattern_SpatFreq_SpatFreq_' strFreq '48Pan.mat'];
